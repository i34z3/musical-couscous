...
Automatic interactive documentation with Swagger UI (from the OpenAPI service): http://localhost/docs

Alternative automatic documentation with ReDoc (from the OpenAPI service): http://localhost/redoc
...
**Note**: The first time you start your stack, it might take a minute for it to be ready. While the service waits for the database to be ready and configures everything. You can check the logs to monitor it.
...
From `./service/app/` you can install all the dependencies with:
...
Next, open your editor at `./service/app/` (instead of the project root: `./`), so that you see an `./app/` directory with your code inside. That way, your editor will be able to find all the imports, etc. Make sure your editor uses the environment you just created with Poetry.

Modify or add SQLAlchemy models in `./service/app/app/models/`, Pydantic schemas in `./service/app/app/schemas/`, API endpoints in `./service/app/app/api/`, CRUD (Create, Read, Update, Delete) utils in `./service/app/app/crud/`. The easiest might be to copy the ones for Items (models, endpoints, and CRUD utils) and update them to your needs.

Add and modify tasks to the Celery worker in `./service/app/app/worker.py`.

If you need to install any additional package to the worker, add it to the file `./service/app/celeryworker.dockerfile`.
...
For example, the directory with the service code is mounted as a Docker "host volume", mapping the code you change live to the directory inside the container. That allows you to test your changes right away, without having to build the Docker image again. It should only be done during development, for production, you should build the Docker image with a recent version of the service code. But during development, it allows you to iterate very fast.
...
There is also a commented out `command` override, you can uncomment it and comment the default one. It makes the service container run a process that does "nothing", but keeps the container alive. That allows you to get inside your running container and execute commands inside, for example a Python interpreter to test installed dependencies, or start the development server that reloads when it detects changes, or start a Jupyter Notebook session.
...
```Bash
DOMAIN=service sh ./scripts/test-local.sh
```
The `./service/app` directory is mounted as a "host volume" inside the docker container (set in the file `docker-compose.dev.volumes.yml`).
You can rerun the test on live code:

```Bash
docker-compose exec service /app/tests-start.sh
```
...
```bash
docker-compose exec service bash /app/tests-start.sh -x
```
...
```Bash
DOMAIN=service sh ./scripts/test-local.sh --cov-report=html
```

To run the tests in a running stack with coverage HTML reports:

```bash
docker-compose exec service bash /app/tests-start.sh --cov-report=html
```
...
You will have a full Jupyter Notebook running inside your container that has direct access to your database by the container name (`db`), etc. So, you can just run sections of your service code directly, for example with [VS Code Python Jupyter Interactive Window](https://code.visualstudio.com/docs/python/jupyter-support-py) or [Hydrogen](https://github.com/nteract/hydrogen).
...
* Start an interactive session in the service container:

```console
$ docker-compose exec service bash
```

* If you created a new model in `./service/app/app/models/`, make sure to import it in `./service/app/app/db/base.py`, that Python module (`base.py`) that imports all the models will be used by Alembic.
...
If you don't want to use migrations at all, uncomment the line in the file at `./service/app/app/db/init_db.py` with:
...
If you don't want to start with the default models and want to remove them / modify them, from the beginning, without having any previous revision, you can remove the revision files (`.py` Python files) under `./service/app/alembic/versions/`. And then create a first migration as described above.
...
In that case, you will need to use a fake local domain (`dev.service_sample.dsbrv.link`) and make your computer think that the domain is is served by the custom IP (e.g. `192.168.99.150`).

If you used the default CORS enabled domains, `dev.service_sample.dsbrv.link` was configured to be allowed. If you want a custom one, you need to add it to the list in the variable `BACKEND_CORS_ORIGINS` in the `.env` file.

* Open your `hosts` file with administrative privileges using a text editor:
  * **Note for Windows**: If you are in Windows, open the main Windows menu, search for "notepad", right click on it, and select the option "open as Administrator" or similar. Then click the "File" menu, "Open file", go to the directory `c:\Windows\System32\Drivers\etc\`, select the option to show "All files" instead of only "Text (.txt) files", and open the `hosts` file.
  * **Note for Mac and Linux**: Your `hosts` file is probably located at `/etc/hosts`, you can edit it in a terminal running `sudo nano /etc/hosts`.

* Additional to the contents it might have, add a new line with the custom IP (e.g. `192.168.99.150`) a space character, and your fake local domain: `dev.service_sample.dsbrv.link`.

The new line might look like:

```
192.168.99.100    dev.service_sample.dsbrv.link
```

* Save the file.
  * **Note for Windows**: Make sure you save the file as "All files", without an extension of `.txt`. By default, Windows tries to add the extension. Make sure the file is saved as is, without extension.

...that will make your computer think that the fake local domain is served by that custom IP, and when you open that URL in your browser, it will talk directly to your locally running server when it is asked to go to `dev.service_sample.dsbrv.link` and think that it is a remote server while it is actually running in your computer.

To configure it in your stack, follow the section **Change the development "domain"** below, using the domain `dev.service_sample.dsbrv.link`.

After performing those steps you should be able to open: http://dev.service_sample.dsbrv.link and it will be server by your stack in `localhost`.

Check all the corresponding available URLs in the section at the end.

### Change the development "domain"

If you need to use your local stack with a different domain than `localhost`, you need to make sure the domain you use points to the IP where your stack is set up. See the different ways to achieve that in the sections above (i.e. using Docker Toolbox with `local.dockertoolbox.tiangolo.com`, using `localhost.tiangolo.com` or using `dev.service_sample.dsbrv.link`).
...
That variable will make your frontend communicate with that domain when interacting with your service API, when the other variable `VUE_APP_ENV` is set to `development`.
...
If you are only developing the frontend (e.g. other team members are developing the service) and there is a staging environment already deployed, you can make your local development code use that staging API instead of a full local Docker Compose stack.
...
That's specially important for a service running a database. But the same problem would apply if you were saving files in your main service service (for example, if those files were uploaded by your users, or if they were created by your system).
...
```bash
STACK_NAME=service_sample-st-dsbrv-link sh ./scripts/deploy.sh
```
...
services:
  db:
    ...
    deploy:
      placement:
        constraints:
          - node.labels.service_sample-dsbrv-link.app-db-data == true
```
...
```bash
docker node update --label-add service_sample-dsbrv-link.app-db-data=true dog.example.com
```

* Then you need to do the same for each stack version you have. For example, for staging you could do:

```bash
docker node update --label-add service_sample-st-dsbrv-link.app-db-data=true cat.example.com
```
...
* Set these environment variables:
  * `DOMAIN=service_sample.dsbrv.link`
  * `TRAEFIK_TAG=service_sample.dsbrv.link`
  * `STACK_NAME=service_sample-dsbrv-link`
  * `TAG=prod`
...
Frontend: https://service_sample.dsbrv.link

Backend: https://service_sample.dsbrv.link/api/

Automatic Interactive Docs (Swagger UI): https://service_sample.dsbrv.link/docs

Automatic Alternative Docs (ReDoc): https://service_sample.dsbrv.link/redoc

PGAdmin: https://pgadmin.service_sample.dsbrv.link

Flower: https://flower.service_sample.dsbrv.link

### Staging URLs

Staging URLs, from the branch `master`.

Frontend: https://service_sample.st.dsbrv.link

Backend: https://service_sample.st.dsbrv.link/api/

Automatic Interactive Docs (Swagger UI): https://service_sample.st.dsbrv.link/docs

Automatic Alternative Docs (ReDoc): https://service_sample.st.dsbrv.link/redoc

PGAdmin: https://pgadmin.service_sample.st.dsbrv.link

Flower: https://flower.service_sample.st.dsbrv.link
...
Frontend: http://dev.service_sample.dsbrv.link

Backend: http://dev.service_sample.dsbrv.link/api/

Automatic Interactive Docs (Swagger UI): https://dev.service_sample.dsbrv.link/docs

Automatic Alternative Docs (ReDoc): https://dev.service_sample.dsbrv.link/redoc

PGAdmin: http://dev.service_sample.dsbrv.link:5050

Flower: http://dev.service_sample.dsbrv.link:5555

Traefik UI: http://dev.service_sample.dsbrv.link:8090
